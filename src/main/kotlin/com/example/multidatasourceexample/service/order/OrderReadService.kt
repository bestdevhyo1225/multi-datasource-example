    ...
    fun findOrders(pageNumber: Int, pageSize: Int): FindListResultDto<FindOrderResultDto> =
        orderReadRepository
            ...
            .let { result: Pair<List<Order>, Long> ->
                val items =
                    result.first.map { order: Order -> FindOrderResultDto.of(order = order, isEmptyOrderItems = true) }

                FindListResultDto(items = items, pageNumber = pageNumber, pageSize = pageSize, total = result.second)
            }
